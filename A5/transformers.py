...
def scaled_dot_product_two_loop_single(
    ...
    e = key.new_zeros(key.shape[0])
    for i in range(query.shape[0]):
        for j in range(key.shape[0]):
            e[j] = torch.dot(query[i], key[j]) / torch.sqrt(torch.tensor(query.shape[1]))
        # Softmax
        e = F.softmax(e, dim=0) # shape (K,)
        out[i] = e @ value
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_batch(
    ...
    K_q = K
    K_k = key.shape[1]
    e = key.new_zeros(N, K_k)
    for i in range(K_q):
        for j in range(K_k):
            e[:, j] = torch.sum(query[:, i, :] * key[:, j, :], dim=1) / torch.sqrt(torch.tensor(M))
            # Softmax
        e = F.softmax(e, dim=1) # shape (N, K_k)
        out[:, i] = torch.bmm(e.unsqueeze(1), value).squeeze() # shape (N, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    e = torch.bmm(query, key.transpose(1, 2)) / torch.sqrt(torch.tensor(M)) # shape (N, K, K)
    if mask is not None:
        ##########################################################################
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        e = e.masked_fill_(mask, -1e9)
    # Replace "pass" statement with your code
    weights_softmax = F.softmax(e, dim=2)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        cqk = torch.sqrt(torch.tensor(6.0) / (dim_in + dim_q))
        cv = torch.sqrt(torch.tensor(6.0) / (dim_in + dim_v))
        nn.init.uniform_(self.q.weight, -cqk, cqk)
        nn.init.uniform_(self.k.weight, -cqk, cqk)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        nn.init.uniform_(self.projection.weight, -c, c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        for attention_layer in self.attention_layers:
            attention_outputs.append(attention_layer(query, key, value, mask))
        attention_outputs = torch.cat(attention_outputs, dim=2)
        y = self.projection(attention_outputs)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.scale = nn.parameter.Parameter(torch.ones(emb_dim))
        self.shift = nn.parameter.Parameter(torch.zeros(emb_dim))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        c = torch.sqrt(torch.tensor(6.0) / (inp_dim + hidden_dim_feedforward))
...
def get_subsequent_mask(seq):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    mask = seq.new_zeros(seq.size(0), seq.size(1), seq.size(1)).to(torch.bool)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.arange(0, K).float() / K
    y = y.repeat(M, 1).T.unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
