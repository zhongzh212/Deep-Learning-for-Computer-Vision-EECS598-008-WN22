...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        padded = torch.nn.functional.pad(x, (pad, pad, pad, pad), 'constant', 0)
        for n in range(N):
            for f in range(F):
                filter = w[f]
                # filter = torch.rot90(filter, 2, (1, 2)) # shape (C, HH, WW)
                for r in range(H_):
                    for c in range(W_):
                        out[n, f, r, c] = b[f] + torch.sum(filter * padded[n, :, r*stride:r*stride+HH, c*stride:c*stride+WW])
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
            for f in range(F):
                for r in range(H_):
                    for c in range(W_):
                        dx[n, :, r*stride:r*stride+HH, c*stride:c*stride+WW] += dout[n, f, r, c] * w[f]
                        dw[f] += dout[n, f, r, c] * padded[n, :, r*stride:r*stride+HH, c*stride:c*stride+WW]
        
        dx = dx[:, :, pad:-pad, pad:-pad] if pad > 0 else dx
    ...
    def forward(x, pool_param):
        ...
        for n in range(N):
            for f in range(C):
                for r in range(H_):
                    for c in range(W_):
                        out[n, f, r, c] = x[n, f, r*stride:r*stride+h, c*stride:c*stride+w].max()
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
            for f in range(C):
                for r in range(H_):
                    for c in range(W_):
                        max_x = x[n, f, r*stride:r*stride+h, c*stride:c*stride+w].max()
                        dx[n, f, r*stride:r*stride+h, c*stride:c*stride+w] += (x[n, f, r*stride:r*stride+h, c*stride:c*stride+w] == max_x) * dout[n, f, r, c]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Replace "pass" statement with your code
        maxpool_out_dim = num_filters * input_dims[1] * input_dims[2] // 4
        self.params['W1'] = weight_scale * torch.randn(num_filters, 
            input_dims[0], filter_size, filter_size, dtype=dtype, device=device
        )
        self.params['b1'] = torch.zeros(num_filters, dtype=dtype, device=device)
        self.params['W2'] = weight_scale * torch.randn(maxpool_out_dim, 
            hidden_dim, dtype=dtype, device=device)
        self.params['b2'] = torch.zeros(hidden_dim, dtype=dtype, device=device)
        self.params['W3'] = weight_scale * torch.randn(hidden_dim, 
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        h1, cache1 = Conv_ReLU_Pool.forward(X, self.params['W1'], 
            self.params['b1'], conv_param, pool_param)
        h2, cache2 = Linear_ReLU.forward(torch.flatten(h1, 1), 
            self.params['W2'], self.params['b2'])
        scores, cache3 = Linear.forward(h2, self.params['W3'], 
            self.params['b3'])
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        ####################################################################
        # Replace "pass" statement with your code
        loss, grads3 = softmax_loss(scores, y)
        grads2, grads['W3'], grads['b3'] = Linear.backward(grads3, cache3)
        _, w3, _ = cache3
        grads['W3'] += 2 * self.reg * w3
        loss += self.reg * torch.sum(w3 * w3)

        grads1, grads['W2'], grads['b2'] = Linear_ReLU.backward(grads2, cache2)
        _, w2, _ = cache2[0]
        grads['W2'] += 2 * self.reg * w2
        loss += self.reg * torch.sum(w2 * w2)

        N = X.shape[0]
        F = self.params['W1'].shape[0]
        h, w = X.shape[2] // 2, X.shape[3] // 2
        grads1 = grads1.reshape(N, F, h, w)
        _, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(grads1, cache1)
        w1, _, _ = cache1   # conv_cache
        w1 = w1[1]
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        #####################################################################
        # Replace "pass" statement with your code
        linear_input_dim = num_filters[-1] * input_dims[1] * input_dims[2]
        F = [input_dims[0]] + num_filters
        ...
        for i, f in enumerate(F):
            if i == 0:
                continue
            if weight_scale == "kaiming":
                self.params['W'+str(i)] = kaiming_initializer(F[i-1], 
                    f, K=3, dtype=dtype, device=device)
            else:   
                self.params['W'+str(i)] = weight_scale * torch.randn(f, 
                    F[i-1], 3, 3, dtype=dtype, device=device)
            self.params['b'+str(i)] = torch.zeros(f, dtype=dtype, device=device)
            if (i-1) in max_pools:
                linear_input_dim //= 4
        ...
        torch.randn(linear_input_dim, num_classes, dtype=dtype, device=device)
        ...
        if batchnorm:
            for i in range(1, self.num_layers):
                self.params['gamma'+str(i)] = torch.ones(F[i], dtype=dtype, 
                    device=device)
                self.params['beta'+str(i)] = torch.zeros(F[i], dtype=dtype, 
                    device=device)
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, num_layers):
            if (i-1) in self.max_pools:
                if self.batchnorm:
                    feature, cache[i] = Conv_BatchNorm_ReLU_Pool.forward(feature, 
                    ...
                    feature, cache[i] = Conv_ReLU_Pool.forward(feature, 
                        self.params['W'+str(i)], self.params['b'+str(i)], 
                        conv_param, pool_param)
            else:
                if self.batchnorm:
                    feature, cache[i] = Conv_BatchNorm_ReLU.forward(feature, 
                        self.params['W'+str(i)], self.params['b'+str(i)], 
                        self.params['gamma'+str(i)], self.params['beta'+str(i)],
                        conv_param, self.bn_params[i-1])
                else:
                    feature, cache[i] = Conv_ReLU.forward(feature, 
                        self.params['W'+str(i)], self.params['b'+str(i)], 
                        conv_param)
        scores, cache[0] = Linear.forward(torch.flatten(feature, 1), 
            self.params['W'+str(num_layers)], self.params['b'+str(num_layers)])
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        dout, grads['W'+str(num_layers)], grads['b'+str(num_layers)] = Linear.backward(dout, cache[0])
        _, w, _ = cache[0]
        grads['W'+str(num_layers)] += 2 * self.reg * w
        loss += self.reg * torch.sum(w * w)

        N = X.shape[0]
        F = self.params['W'+str(num_layers-1)].shape[0]
        H, W = X.shape[2] // 2 ** len(self.max_pools), X.shape[3] // 2 ** len(self.max_pools)
            ...
            if (L-1) in self.max_pools:
                if self.batchnorm:
                    dout, grads['W'+str(L)], grads['b'+str(L)], grads['gamma'+str(L)], grads['beta'+str(L)] = \
                        Conv_BatchNorm_ReLU_Pool.backward(dout, cache[L])
                    w, _, _, _ = cache[L]
                else:
                    dout, grads['W'+str(L)], grads['b'+str(L)] = Conv_ReLU_Pool.backward(dout, cache[L])
                    w, _, _ = cache[L]
            else:
                if self.batchnorm:
                    dout, grads['W'+str(L)], grads['b'+str(L)], grads['gamma'+str(L)], grads['beta'+str(L)] = \
                        Conv_BatchNorm_ReLU.backward(dout, cache[L])
                    w, _, _ = cache[L]
                else:
                    dout, grads['W'+str(L)], grads['b'+str(L)] = Conv_ReLU.backward(dout, cache[L])
                    w, _ = cache[L]
            w = w[1]
            grads['W'+str(L)] += 2 * self.reg * w
            loss += self.reg * torch.sum(w * w)
            L -= 1
        #############################################################
        #                       END OF YOUR CODE                    #
...
def find_overfit_parameters():
    ...
    ###########################################################
    # Replace "pass" statement with your code
    weight_scale, learning_rate = 0.1, 2e-3   # Experiment with this!
    ###########################################################
    #                       END OF YOUR CODE                  #
    ###########################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict,
                optim_config={'learning_rate': 0.3}, 
                lr_decay=0.9, batch_size=128, device=device,
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.sqrt(torch.tensor(gain / Din)) * torch.randn(Din, Dout, 
            dtype=dtype, device=device)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.sqrt(torch.tensor(gain / (Din * K * K))) * torch.randn(
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            x_mu = x - mu
            sq = x_mu * x_mu
            var = torch.sum(sq, axis=0) / N
            sigma = torch.sqrt(var + eps)
            inv_sigma = 1 / sigma
            x_hat = x_mu * inv_sigma
            out = x_hat * gamma + beta
            running_mean *= momentum
            # print(running_mean.shape, mu.shape)
            running_mean += (1 - momentum) * mu
            running_var *= momentum
            running_var += (1 - momentum) * var
            cache = (x_hat, gamma, x_mu, var, eps, mode, sigma)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x_hat, gamma, x_mu, var, eps, mode, sigma = cache
        N, D = dout.shape
        dbeta = torch.sum(dout, 0)  # shape (D,)
        if mode == 'train':
          dx_hat = dout * gamma # shape (N, D)
          dvar = torch.sum(dx_hat * x_mu, axis=0) * (-0.5 / torch.sqrt(var + eps) / (var+eps)) # shape (D,)
          dmu_1 = dx_hat / torch.sqrt(var + eps) # shape (N, D)
          dmu_2 = dvar / N * 2 * x_mu
          dmu = -torch.sum(dmu_1 + dmu_2, axis=0)
          dx = dmu_1 + dmu_2 + dmu / N
          dgamma = torch.sum(dout * x_hat, axis=0) # shape (D,)
          dbeta = torch.sum(dout, 0)  # shape (D,)
        elif mode == 'test':
          dx = dout * gamma / torch.sqrt(var + eps)
          dgamma = torch.sum(dout * x_mu, axis=0) / torch.sqrt(var + eps)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        if mode == 'train':
          ...
          dx = gamma * (dout - dbeta / N - x_hat * dgamma / N) / sigma
        elif mode == 'test':
          dx = dout * gamma / torch.sqrt(var + eps)
          dgamma = torch.sum(dout * x_mu, axis=0) / torch.sqrt(var + eps)
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        ################################################################
        # Replace "pass" statement with your code
        x_tr = torch.transpose(x, 1, 3) # shape (N, W, H, C)
        sh = x_tr.shape # (N, W, H, C)
        x_tr = x_tr.reshape(-1, x.shape[1])
        out, cache = BatchNorm.forward(x_tr, gamma, beta, bn_param)
        out = out.reshape(sh).transpose(1, 3)
    ...
    def backward(dout, cache):
        ...
        #################################################################
        # Replace "pass" statement with your code
        dout_tr = torch.transpose(dout, 1, 3)
        sh = dout_tr.shape
        dout_tr = dout_tr.reshape(-1, dout.shape[1])
        dx, dgamma, dbeta = BatchNorm.backward(dout_tr, cache)
        dx = dx.reshape(sh).transpose(1, 3)
        ##################################################################
        #                       END OF YOUR CODE                         #
        ##################################################################
