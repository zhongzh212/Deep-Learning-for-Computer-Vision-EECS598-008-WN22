...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        self.relu = ReLU()
        self.fc2 = Linear()
        out1, fc1_cache = self.fc1.forward(X, self.params['W1'], self.params['b1'])
        hidden, relu_cache = self.relu.forward(out1)
        scores, fc2_cache = self.fc2.forward(hidden, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        ###################################################################
        # Replace "pass" statement with your code
        loss, grads_fc2 = softmax_loss(scores, y)
        grads_relu1, grads['W2'], grads['b2'] = self.fc2.backward(grads_fc2, fc2_cache)
        _, w2, _ = fc2_cache
        grads['W2'] += 2 * self.reg * w2
        loss += self.reg * torch.sum(w2 * w2)

        grads_fc1 = self.relu.backward(grads_relu1, relu_cache)

        _, grads['W1'], grads['b1'] = self.fc1.backward(grads_fc1, fc1_cache)
        _, w1, _ = fc1_cache
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, len(temp)):
            self.params["W"+str(i)] = weight_scale * torch.randn(
                temp[i-1], temp[i], dtype=dtype, device=device
            )
            self.params["b"+str(i)] = torch.zeros(temp[i], dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        L = self.num_layers
        for i in range(1, L):
            features, cache['fc'+str(i)] = Linear.forward(features, self.params['W'+str(i)], self.params['b'+str(i)])
            features, cache['relu'+str(i)] = ReLU.forward(features)
            if self.use_dropout: 
                features, cache['drop'+str(i)] = Dropout.forward(features, self.dropout_param)
        scores, cache['fc_final'] = Linear.forward(features, self.params['W'+str(L)], self.params['b'+str(L)])
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        #####################################################################
        # Replace "pass" statement with your code
        loss, grads_final = softmax_loss(scores, y)
        grads_relu, grads['W'+str(L)], grads['b'+str(L)] = Linear.backward(grads_final, cache['fc_final'])
        _, WL, _ = cache['fc_final']
        grads['W'+str(L)] += 2 * self.reg * WL
        loss += self.reg * torch.sum(WL * WL)
        L -= 1
        while L > 0:
            if self.use_dropout:
                grads_relu = Dropout.backward(grads_relu, cache['drop'+str(L)])
            grads_fc = ReLU.backward(grads_relu, cache['relu'+str(L)])
            grads_relu, grads['W'+str(L)], grads['b'+str(L)] = Linear.backward(grads_fc, cache['fc'+str(L)])
            _, WL, _ = cache['fc'+str(L)]
            grads['W'+str(L)] += 2 * self.reg * WL
            loss += self.reg * torch.sum(WL * WL)
            L -= 1
        ###########################################################
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict, 
                ...
                lr_decay = 0.95, batch_size=32, device=device,
                num_epochs=8, print_every=250)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def adam(w, dw, config=None):
    ...
    config['t'] += 1
    m1_unbias = config['m'] / (1 - config['beta1'] ** config['t'])
    m2_unbias = config['v'] / (1 - config['beta2'] ** config['t'])
    next_w = w - config['learning_rate'] * m1_unbias / (m2_unbias.sqrt() + config["epsilon"])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
