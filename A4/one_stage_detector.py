    ...
    def forward(
        ...
        for i in range(B):
            ...
            for key in matched_gt_boxes[i]:
                matched_gt_deltas[i][key] = fcos_get_deltas_from_locations(
                    locations_per_fpn_level[key], matched_gt_boxes[i][key], stride=self.backbone.fpn_strides[key]
                )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # print('mgb', matched_gt_boxes.shape) (B, sum_resolution, 5)
        ...
        # shape (batch_size, sum_resolution)
