...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = torch.maximum(hidden, torch.tensor(0))
...
def nn_forward_backward(
    ...
    loss = 0.0
    num_train = X.shape[0]
    scores_max, _ = scores.max(1, True)
    scores -= scores_max
    loss -= torch.sum(scores[torch.arange(num_train), y])
    scores = scores.exp()
    sum_scores = torch.sum(scores, axis=1, keepdim=True)  # shape (N, 1)
    loss += torch.sum(sum_scores.log())
    loss /= N
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    dg = scores / sum_scores  # shape (N, C)
    dg[torch.arange(N), y] -= 1
    dW2 = h1.T.matmul(dg) # shape (H, C)
    dW2 /= num_train
    dW2 += 2 * reg * W2
    grads['W2'] = dW2
    db2 = dg.sum(axis=0)
    db2 /= num_train
    grads['b2'] = db2
    dh = dg.matmul(W2.T)  # shape (N, H)
    df = dh * torch.where(h1 > 0, 1, 0)  # shape (N, H)
    dW1 = X.T.matmul(df) # shape (D, H)
    dW1 /= num_train
    dW1 += 2 * reg * W1
    grads['W1'] = dW1
    db1 = df.sum(axis=0)
...
def find_best_net(
    ...
    D = X_train.shape[1]
    C = torch.max(y_train) + 1
    lrs, Hs, regs, lr_decays = get_param_set_fn()
    for learning_rate in lrs:
        for H in Hs:
            for reg in regs:
                for learning_rate_decay in lr_decays:
                    cls = TwoLayerNet(D, H, C)
                    history = cls.train(
                        X_train, y_train, X_val, y_val, learning_rate, 
                        learning_rate_decay, reg, 2500
                    )
                    val_acc = history['val_acc_history'][-1]
                    print(learning_rate, H, reg, learning_rate_decay, val_acc)
