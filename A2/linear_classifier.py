...
def svm_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    scores = X.matmul(W)
    correct_scores = scores[torch.arange(scores.shape[0]), y]   # shape (N,)
    margins = torch.maximum(torch.tensor(0), scores - correct_scores.reshape(-1, 1) + 1)
    margins[torch.arange(scores.shape[0]), y] = 0
    loss = torch.mean(torch.sum(margins, 1)) + reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    mask[margins > 0] = 1
    tmp = torch.sum(mask, axis=1)
    mask[torch.arange(X.shape[0]), y] = -tmp
    dW = torch.matmul(X.transpose(0, 1), mask)
    dW /= X.shape[0]
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [2e-3, 5e-3, 8e-3, 0.01]
    regularization_strengths = [4e-4, 0.001, 0.004, 0.01, 0.04]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    train_acc = y_pred_train == data_dict['y_train']
    train_acc = train_acc.sum() / train_acc.shape[0]

    y_pred_val = cls.predict(data_dict['X_val'])
    val_acc = y_pred_val == data_dict['y_val']
    val_acc = torch.sum(val_acc) / val_acc.shape[0]
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_vectorized(
    ...
    f = X.matmul(W)
    f_max, _ = f.max(1, True)
    f -= f_max
    loss -= torch.sum(f[torch.arange(N), y])
    f = f.exp()
    sum_f = torch.sum(f, axis=1, keepdim=True)  # shape (N,)
    loss += torch.sum(sum_f.log())
    loss /= N
    loss += reg * torch.sum(W * W)

    dW = f / sum_f  # shape (N, C)
    dW[torch.arange(N), y] -= 1
    dW = X.T.matmul(dW) # shape (D, C)
    dW /= N
    dW += 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [3e-3, 5e-3, 8e-3, 0.01]
    regularization_strengths = [4e-5, 1e-4, 4e-3, 1e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
